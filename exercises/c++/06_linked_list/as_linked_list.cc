...
class List {
	...
	private:
		//struct node, it has a templated value and a pointer to the next node
		struct node {
			...
			//which kind of constructor is this???
		...
		template <class OT>
		void push_back(OT&& v);
		//add a node at the head of the queue, non trivial version
		template <class OT>
